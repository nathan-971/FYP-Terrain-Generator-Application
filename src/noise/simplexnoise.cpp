...
const int SimplexNoise::permutation[256] = {
...
void SimplexNoise::initPermTable()
{
...
float SimplexNoise::Get(float x, float y, float freq)
{
    initPermTable();

    const float skewFactor = (sqrt(3) - 1) * 0.5;
    const float unskewFactor = (3 - sqrt(3)) / 6;

    float xf = x * freq;
    float yf = y * freq;

    float s = (xf + yf) * skewFactor;
    int i = (int)floor(x + s);
    int j = (int)floor(y + s);
...
float SimplexNoise::dotProduct(const Vector& gradient, const Vector& gridPoint)
{
